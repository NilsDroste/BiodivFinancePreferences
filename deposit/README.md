...
**Withheld variables:** the panel provider identifier (`pid`), the open-ended responses (`q17`), and the postal-code item (`q9`). The postal-code item asked for two digits but was often answered with a full five-digit code; combined with gender, income and party preference it left a majority of respondents uniquely identifiable. It is not used in any analysis. With it removed, 0.4 per cent of the sample is unique on gender, income and party preference.
